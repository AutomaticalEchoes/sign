...
public class ClientGetMarkCommand {
    ...
    public static boolean CheckMark(CommandSourceStack sourceStack, Sign mark){
        if(!mark.CanUse()){
            sourceStack.sendFailure(new TranslatableComponent("sign.source_discord"));
            return false;
        }
        if(ClientEvents.SIGNS.contains(mark)){
            sourceStack.sendFailure(new TranslatableComponent("sign.exist"));
            return false;
        }
        if(mark instanceof EntitySign entitySign && entitySign.isLocalPlayer()){
            sourceStack.sendFailure(new TranslatableComponent("sign.self"));
            return false;
        }
