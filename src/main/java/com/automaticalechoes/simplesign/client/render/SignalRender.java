...
public class SignalRender {
    private static final ModelResourceLocation TRIDENT_MODEL = new ModelResourceLocation("minecraft:trident#inventory");
    private static final ModelResourceLocation SPYGLASS_MODEL = new ModelResourceLocation("minecraft:spyglass#inventory");
    ...
    public static void RenderMark(com.automaticalechoes.simplesign.common.sign.Sign mark, PoseStack poseStack, Camera camera, Matrix4f projectionMatrix){
        ...
        MutableComponent distance = new TranslatableComponent(DECIMAL_FORMAT.format(length)).append(new TextComponent("B").withStyle(ChatFormatting.GOLD));
...
//    }
    ...
    public static void RenderPoint(Vec3 pointPos, PoseStack poseStack, Camera camera, Component distanceMessage,Color pointColor ,float scale, Matrix4f projectionMatrix, @Nullable ItemStack itemStack){
        ...
        minecraft.font.drawInBatch(distanceMessage,- width / 2F ,10,-1,false,poseStack.last().pose(),minecraft.renderBuffers().bufferSource(),true,0, 15728880);
        poseStack.popPose();
    }

    public static void RenderPointTexture(PoseStack p_114083_, MultiBufferSource p_114084_, int p_114085_, Color color) {
        p_114083_.pushPose();
        p_114083_.mulPose(Vector3f.ZP.rotationDegrees(45.0F));
    ...
    public static void renderItem(ItemStack p_115144_,  PoseStack p_115147_, MultiBufferSource p_115148_) {
        ...
        if (!p_115144_.isEmpty()) {
            ...
            model = net.minecraftforge.client.ForgeHooksClient.handleCameraTransforms(p_115147_, model, ItemTransforms.TransformType.GUI, false);
            p_115147_.translate(- 0.5F, 0, - 0.5F);
            if(!model.usesBlockLight()) Lighting.setupForFlatItems();
            RenderType renderType = Utils.getRenderType(p_115144_, true);
